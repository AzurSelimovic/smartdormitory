<?php
    ...
    if(empty($_SESSION['mUserId'])  && empty($_SESSION['password']))
        ...
        $log_query = mysqli_query($conn,"SELECT * FROM action_log ORDER BY id DESC");
...
<body>
        <sesction id='dashboard'>
            ...
            <div id='main'>
                <table>
                <tr><th>Log details table:</th><th>Date and Time</th></tr>";
                while($action_r = mysqli_fetch_array($log_query)){
                    echo "<tr><td>" . $action_r['action'] ."</td><td>". $action_r['timestamp'] . "</td></tr>";
                }
                echo"
            </div>
        </sesction>
