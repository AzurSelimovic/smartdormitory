<?php
    ...
    if(empty($_SESSION['mUserId'])  && empty($_SESSION['password']))
        ...
        $sql0 = mysqli_query($conn,"SELECT * FROM user WHERE fname = '$ime' and lname = '$prezime'");
        $row0 = mysqli_fetch_array($sql0);
        $targetid = $row0['id'];
        $targetfName = $row0['fname'];
        $targetLname = $row0['lname'];
        $_SESSION['targetid'] = $targetid;
        $_SESSION['targetfName'] = $targetfName;
        $_SESSION['targetLname'] = $targetLname;
        $sql1 = mysqli_query($conn,"SELECT * FROM subjects WHERE mUserId = '$targetid'");
...
<body>
    ...
    echo "<tr><td>".$row0['fname']." ".$row0['lname']."</td><td>" . $row1['subjectName'] . "</td>" . "<td>" . $row1['mark'] . "</td><td>" . $row1['date'] . "</td></tr>";
}
echo"
                </table>
                <br>
                <form action='edit_subjects.php' method='post'>
                        <input type='text' name='ime' value='$targetfName' readonly>
                        <input type='text' name='prezime' value='$targetLname' readonly>
                        <input type='text' name='subject_name' placeholder='Unesite naziv predmeta...' required>
