<?php
    ...
    if(empty($_SESSION['mUserId'])  && empty($_SESSION['password']))
        ...
        $sql1 = mysqli_query($conn,"SELECT * FROM user WHERE type='student'");
        $brojac = 0;
...
<body>
        <sesction id='dashboard'>
            ...
            <div id='main'>
            <table>
            <tr><th>-</th><th>Ime </th><th>Prezime</th><th>Učitaj više o</th></tr>
                    ";
                    while($row1 = mysqli_fetch_array($sql1)){
                        $brojac = $brojac + 1 ;
                        $linkName = $row1['fname'];
                        $linkSurname = $row1['lname'];
                        echo "<tr><td>".$brojac ."</td><td>". $row1['fname'] ."</td><td>". $row1['lname']."</td><td><a style='color=black' href='details.php?linkName=$linkName&linkSurname=$linkSurname'><button type='button' class='btn btn-info'>Učitaj podatke</button></a></td></tr>";
                    }
            echo"
            </table></div>
        </sesction>
