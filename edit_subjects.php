<?php
    ...
    if(empty($_SESSION['mUserId'])  && empty($_SESSION['password']))
        ...
        $ime = $_SESSION['targetfName'];
        $prezime = $_SESSION['targetLname'];
        $sql0 = mysqli_query($conn,"SELECT * FROM user WHERE fname = '$ime' and lname = '$prezime'");
        $row0 = mysqli_fetch_array($sql0);
        $targetid = $row0['id'];
        $sql1 = mysqli_query($conn,"SELECT mark FROM subjects WHERE mUserId = '$targetid' and subjectName = '$subject_name'");
...
<body>
        <sesction id='dashboard'>
            ...
            <div id='main'>
                ...
                <form action='edit_subjects_r.php' method='post'>
                        ...
                        <input type='text' name='subject_edit' value='$mark' required>
